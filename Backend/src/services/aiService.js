...
const getClient = () => {
    ...
    if (!key || key === "your-gemini-api-key"){
        throw new ApiError(503,"Gemini API is not configured on the server");
    }
    if (!client){
...
const generateTasks = async (goal, count = 6) => {
    const prompt = `You are a senior project manager. Break the following project goal into ${count} concrete, accountable kanban tasks
    Goal: "${goal}" 
    
    Respond ONLY with a JSON array. Each item: {"title" : string, "description": string (1-2 sentence), "priority": "low"|"medium"|"high"|"urgent"}.
...
const summarizeBoard = async({boardTitle, columns}) => {
    const snapshot = columns.map(
        (c) => `${c.title} (${c.tasks.length}) :\n` + 
                (c.tasks.map((t) => ` - ${t.title} [${t.priority}]`).join("\n") || " (none)")
    ).join("\n");

    const prompt = `You are a scrum master. Write a concrete sprint summary for the kanban board "${boardTitle}".
    Current board state:
    ${snapshot}
